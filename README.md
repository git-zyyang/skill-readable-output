...
你刚花两小时和 Claude 讨论一个系统架构问题。探索了三种方案，否决了两种，在一个意想不到的地方找到了关键约束，最终选定的方案要求你改变对整个系统的心智模型。

明天你还记得结论。三个月后，你不会记得为什么否决了另外两种方案。你会重新探索它们。你会再浪费一个下午。

**这个 Skill 把那段对话变成一个自包含的 HTML 页面。重新打开，5分钟回到当时的思考现场。**

不是转录。不是摘要。是一份决策记录。决定了什么、为什么、否决了什么、还有什么悬而未决。
...
这比没有对话更糟。因为你以为自己记录了，"在聊天记录里呢"，所以你永远不会正经写下来。
...
Skill 问 2-3 个问题，生成 HTML，自动在浏览器打开。30秒完成安装，一个词触发生成。
...
| 不可执行 | 提取具体 TODO 和优先级 |
| 没有边界 | 声明结论在什么条件下可能不成立 |

## 内置质量门控

Skill 不只是写。它审计自己：

- **3 点上限**：超过 3 个核心洞察就合并或降级。工作记忆是真实的约束。
- **砍半测试**：只保留一半，哪些活下来？活下来的就是骨架。
- **体量守卫**：选了完整版但只有 3 个决策点？Skill 会反劝你降到标准版。
- **4 问自检**：脱离对话能看懂？决策和理由都在？不是转录？TODO 可执行？
- **11 条反模式**：转录式堆砌、注水填充、模糊 TODO、缺失理由，全部列入黑名单。

## 自定义

**加风格**：在 Skill 文件的 CSS token 部分加一个 `:root` 块。HTML 骨架共用，只有变量不同。

**加场景**：在结构表中加一行。每个场景需要名称、结构模式、可选的默认风格。
...
- 移植到其他 AI 助手：GPT、Gemini
- 真实产出示例放到 `examples/` 目录
- 翻译
